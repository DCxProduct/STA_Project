...
class Helper
{
    ...
    static function SaveVisitorInfo($PageTitle, $visitor_ip, $device_id)
    {
        if (config('smartend.geoip_status')) {
            ...
            // $SavedVisitor = AnalyticsVisitor::where('ip', '=', $visitor_ip)->where('referrer', '=', $device_id)->where('date', '=', date('Y-m-d'))->first();
            // if (empty($SavedVisitor) || @$SavedVisitor->country == "unknown") {
                ...
                $exists = AnalyticsVisitor::where('ip', '=', $visitor_ip)
                        ->where('referrer', '=', $device_id)
                        ->where('date', '=', date('Y-m-d'))
                        ->first();

                // Start saving page info to database
                $VisitedPage = new AnalyticsPage;
                $VisitedPage->visitor_id = $exists->id;
