        ...
        <div class="box-body">
            @if (Session::has('commentST'))
                @if (Session::get('commentST') == "create")
                    ...
                    <div>
                        {{Form::open(['route'=>['topicsCommentsStore',$WebmasterSection->id,$Topic->id],'method'=>'POST','enctype' => 'multipart/form-data','files' => true ])}}
                        ...
                        <div class="form-group row">
                            <label for="comment_document" class="col-sm-2 form-control-label">
                                {!! __('backend.topicDocument') !!}
                            </label>
                            <div class="col-sm-10">
                                {!! Form::file('document', ['class' => 'form-control', 'id' => 'document', 'accept' => 'application/pdf', 'required' => '']) !!}
                            </div>
                        </div>
                        <div class="form-group row">
                            <label for="comment"
                ...
                @if (Session::get('commentST') == "edit")
                    <div>
                        {{Form::open(['route'=>['topicsCommentsUpdate',$WebmasterSection->id,$Topic->id,Session::get('Comment')->id],'method'=>'POST','enctype' => 'multipart/form-data','files' => true ])}}
                        ...
                        <div class="form-group row">
                                <label for="attach_file"
                                       class="col-sm-2 form-control-label">{!!  __('backend.topicDocument') !!}</label>
                                <div class="col-sm-10">
                                    @if(Session::get('Comment')->document!="")
                                        <div id="topic_attach" class="col-sm-4 box p-a-xs">
                                            <a target="_blank"
                                               href="{{ asset('uploads/feedbacks/'.Session::get('Comment')->document) }}"> {{ Session::get('Comment')->document }} </a>
                                            <br>
                                    ...
                                    @endif
                                    {!! Form::file('document', array('class' => 'form-control','id'=>'document','accept'=>"application/pdf")) !!}
                                </div>
                        </div>
                        <div class="form-group row">
                            <label for="comment"
                ...
                @if(count($Topic->comments)>0)
                    ...
                    <table class="table table-bordered">
                        ...
                        <tr>
                            ...
                            <th>{{ __('backend.topicDocument') }}</th>
                        ...
                        @foreach($Topic->comments as $comment)
                            <tr>
                                ...
                                <td>
                                    <a target="_blank"
                                        href="{{ asset('uploads/feedbacks/'. $comment->document) }}"> {{ $comment->document }} 
                                    </a>
                                </td>
