                ...
                @foreach(Helper::languagesList() as $ActiveLanguage)
                    @if($ActiveLanguage->box_status)
                        <div class="form-group row">
                            <label
                                class="col-sm-2 form-control-label">{!!  __('backend.bannerTitle') !!} <span style="color:red;">*</span> {!! @Helper::languageName($ActiveLanguage) !!}
                            </label>
                            <div class="col-sm-10">
                    ...
                    <div id="files_div">
                        @foreach(Helper::languagesList() as $ActiveLanguage)
                            @if($ActiveLanguage->box_status)
                                <div class="form-group row">
                                    <label
                                        class="col-sm-2 form-control-label">{!!  __('backend.'.$ttile) !!} <span style="color:red;">*</span> {!! @Helper::languageName($ActiveLanguage) !!}
                                    </label>
                                    <div class="col-sm-10">
                                        {!! Form::file($file_name.$ActiveLanguage->code, array('class' => 'form-control','accept'=>$file_allow,'required'=>'')) !!}
                                    </div>
                                </div>
                            @endif
                        @endforeach
                ...
                @if($WebmasterBanner->desc_status)
                    @foreach(Helper::languagesList() as $ActiveLanguage)
                        @if($ActiveLanguage->box_status)
                            <div class="form-group row">
                                <label
                                    class="col-sm-2 form-control-label">{!!  __('backend.bannerDetails') !!} <span style="color:red;">*</span> {!! @Helper::languageName($ActiveLanguage) !!}
                                </label>
                                <div class="col-sm-10">
                                    {!! Form::textarea('details_'.$ActiveLanguage->code,'', array('placeholder' => '','required'=>'','class' => 'form-control', 'dir'=>@$ActiveLanguage->direction,'rows'=>'3')) !!}
                                </div>
                            </div>
                        @endif
                    @endforeach
                ...
                @if($WebmasterBanner->link_status)
                    @foreach(Helper::languagesList() as $ActiveLanguage)
                        @if($ActiveLanguage->box_status)
                            <div class="form-group row">
                                <label
                                    class="col-sm-2 form-control-label">{!!  __('backend.bannerLinkUrl') !!} <span style="color:red;">*</span> {!! @Helper::languageName($ActiveLanguage) !!}
                                </label>
                                <div class="col-sm-10">
                                    {!! Form::text('link_'.@$ActiveLanguage->code,'', array('placeholder' => 'http://www.site.com','required'=>'','class' => 'form-control','maxlength'=>191, 'dir'=>'ltr')) !!}
                                </div>
                            </div>
                        @endif
                    @endforeach
