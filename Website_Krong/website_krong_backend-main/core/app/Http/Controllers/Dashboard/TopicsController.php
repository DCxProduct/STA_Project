...
class TopicsController extends Controller
{
    ...
    private string $uploadPathComment = "uploads/feedbacks/";
    ...
    public function list(Request $request)
    {
        ...
        if ($totalFiltered > 0) {
            ...
            foreach ($Topics as $Topic) {
                    ...
                    if (in_array(Auth::user()->permissions_id, $view_permission_groups) || in_array(0, $view_permission_groups) || $customField->view_permission_groups == "") {
                        ...
                        if (($customField->lang_code == "all" || $customField->lang_code == @Helper::currentLanguage()->code)) {
                            if ($customField->type == 12) {
                                ...
                                if (trim($cf_saved_val) != "") {
                                    $cf_data = "<a target='_blank' href='" . URL::to('uploads/topics/' . $cf_saved_val) . "'><i class='fa fa-paperclip'></i></a><span style='text-indent: 50px;cursor: pointer;' onclick=\"copyText('" . URL::to('uploads/topics/' . $cf_saved_val) . "')\"><i class='fa fa-copy'></i></span>";
                                }
                ...
                //set manually permission
                $bool = true; 
                $section_feedback = config('dev.app.feedback_section');
                $section_welcome  = config('dev.app.welcome_section');
                $contact_page        = config('dev.app.contact_us_page');
                $about_page          = config('dev.app.about_page');
                $cover_page_job      = config('dev.app.cover_page_job');
                $cover_page_tourism      = config('dev.app.cover_page_tourism');
                $cover_page_legal_document      = config('dev.app.cover_page_legal_document');
                $cover_page_news      = config('dev.app.cover_page_news');
                $cover_page_scholarships      = config('dev.app.cover_page_scholarships');
                $cover_page_one_window_service      = config('dev.app.cover_page_one_window_service');
                if($Topic->webmasterSection->id == $section_feedback || $Topic->webmasterSection->id == $section_welcome){
                    $bool = false; 
                }
        ...
        <div class="dropdown-menu pull-right">
          <a class="dropdown-item" href="' . ((@$Topic->webmasterSection->type == 4 || @$Topic->webmasterSection->type == 6 || @$Topic->webmasterSection->type == 7 || !@$Topic->status) ? route("topicView", ["webmasterId" => @$Topic->webmasterSection->id, "id" => $Topic->id]) : Helper::topicURL($Topic->id, '', $Topic)) . '" ' . ((@$Topic->webmasterSection->type == 4 || @$Topic->webmasterSection->type == 6 || @$Topic->webmasterSection->type == 7) ? "" : "target='_blank'") . '><i class="material-icons">&#xe8f4;</i> ' . __('backend.preview') . '</a>';
                ...
                if($bool){
                    ...
                    if($Topic->id != $contact_page && $Topic->id != $about_page && $Topic->id != $cover_page_job && $Topic->id != $cover_page_tourism && $Topic->id != $cover_page_legal_document && $Topic->id != $cover_page_news && $Topic->id != $cover_page_scholarships && $Topic->id != $cover_page_one_window_service){
                    ...
                    if (in_array(Auth::user()->permissions_id, $add_permission_groups) || in_array(0, $add_permission_groups) || $customField->add_permission_groups == "") {
                        ...
                        if ($request->$field_value_var != "") {
                            if ($customField->type == 8 || $customField->type == 9 || $customField->type == 10) {
                                // upload file
                                if ($request->$field_value_var != "") {
                                     $validate_customField = [
                                        $field_value_var => 'mimes:' . $this->allowed_file_types
                                    ];
                                    $this->validate($request, $validate_customField);
    ...
    public function commentsStore(Request $request, $webmasterId, $id)
    {
        ...
        if (!empty($WebmasterSection)) {
            ...
            //Start of Upload Files
            $formFileName = "document";
            $attachFileFinalName = "";
            if ($request->$formFileName != "") {
                 $attachFileFinalName = time() . rand(1111, 9999) . '.' . $request->file($formFileName)->getClientOriginalExtension();
                 $path = $this->uploadPathComment;
                 $request->file($formFileName)->move($path, $attachFileFinalName);
            ...
            $Comment->document = strip_tags($attachFileFinalName);
            $Comment->topic_id = $id;
    ...
    public function commentsUpdate(Request $request, $webmasterId, $id, $comment_id)
    {
        ...
        if (!empty($WebmasterSection)) {
            ...
            if (!empty($Comment)) {
                ...
                if ($request->$formFileName != "") {
                    // Delete file if there is a new one
                    if ($Comment->$formFileName != "" && $Comment->$formFileName != "default.png") {
                        File::delete($this->uploadPathComment . $Comment->$formFileName);
                    }
                    $attachFileFinalName = time() . rand(1111,
                            9999) . '.' . $request->file($formFileName)->getClientOriginalExtension();
                    $path = $this->uploadPathComment;
    ...
    public function commentsDestroy($webmasterId, $id, $comment_id)
    {
        ...
        if (!empty($WebmasterSection)) {
            ...
            if (!empty($Comment)) {
                // Delete file if there is a new one
                if ($Comment->document != "" && $Comment->document != "default.png") {
                    File::delete($this->uploadPathComment . $Comment->document);
                }
                $Comment->delete();
                return redirect()->action('Dashboard\TopicsController@edit', [$webmasterId, $id])->with('doneMessage',
...
<i class=\"dark-white\"></i> &nbsp;<label for=\"related_topics_$i\">$title</label>
    ...
    public function builder($topic_id){
        $lang = \request()->input('lang');
        if ($lang == "") {
            $lang = @Helper::currentLanguage()->code;
        }
        ...
        if (!empty($Topic)) {
            ...
            return view("dashboard.topics.builder.edit", compact("Topic", "title", "content", "lang"));
        } else {
            return redirect()->route('NotFound');
        }
    }
