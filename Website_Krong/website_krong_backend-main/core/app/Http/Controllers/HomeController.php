...
class HomeController extends Controller
{
    ...
    {
        return redirect('/admin'); //add custom redirect to admin page
    ...
    public function list_page($lang, $WebmasterSection = [], $Category = [])
    {
        if (!empty($WebmasterSection)) {
            ...
            // return view
            $view = "topics";
            if($WebmasterSection->type == 0 && $WebmasterSection->id == 15){
                $view = "research-centers";
            }
            elseif ($WebmasterSection->type == 9) {
                $view = "docs";
    ...
    public function comment_submit(Request $request)
    {
        $validation = [
            ...
            'comment_document' => 'required|file|mimes:pdf|max:2048'
        ];
        if (config('smartend.nocaptcha_status')) {
            $validation['g-recaptcha-response'] = 'required|captcha';
        }
        ...
        if ($validator->passes()) {
            ...
            if (!empty($Topic)) {
                ...
                if (@Helper::GeneralSiteSettings('notify_comments_status')) {
                    try {
                        $recipient = explode(",", str_replace(" ", "", $site_email));
                        //$message_details = __('frontend.name') . ": " . strip_tags($request->comment_name) . "<hr>" . __('frontend.document') . ": " . strip_tags(asset('uploads/feedbacks/' . $attachFileFinalName)) . "<hr>" . __('frontend.comment') . ":<br>" . nl2br(strip_tags($request->comment_message));
