...
class Helper
{
    ...
    static function SaveVisitorInfo($PageTitle)
    {
        if (config('smartend.geoip_status')) {
            ...
            $timeLimit = now()->subMinutes(20)->format('Y-m-d H:i:s');
            $SavedVisitor = AnalyticsVisitor::where('date', '=', date('Y-m-d'))  
                            ->where('ip', '=', $visitor_ip)   
            ...
            if (!($SavedVisitor)) {
                ...
                try {
                    $visitor_local_ip_details = AnalyticsVisitor::where('date', '=', date('Y-m-d'))->where('ip', $visitor_ip)->first();
                ...
                $timeLimitUnit = now()->format('Y-m-d H:i:s');
                $SavedUnitVisitor =AnalyticsVisitor::where('date', '=', date('Y-m-d'))  
                                    ->where('ip', '=', $visitor_ip)   
                                    ->where('created_at', '=', $timeLimitUnit)
                                    ->first();     
                if(!$SavedUnitVisitor){                        
                ...
                $Savedpage = AnalyticsPage::where('date', '=', date('Y-m-d'))                            
                            ->where('ip', '=',$visitor_ip)
                            ->where('query', '=', $current_page_full_link)                                    
                            ->first();
                if (!($Savedpage) && (strpos($current_page_full_link, 'page')  !== false || strpos($current_page_full_link, 'info')  !== false)) {
    ...
    static function dateForDB($date = "", $withTime = 0)
    {
        if ($date != "") {            
            try {
                $format = config('smartend.date_format');                
                if ($withTime) {
                    return Carbon::createFromFormat($format . " h:i:s", $date)->format('Y-m-d H:i:s');
                } else {                    
                    return Carbon::createFromFormat($format, $date)->format('Y-m-d H:i');
                }
            } catch (\Exception $e) {
                return $date;
            }
    ...
    static function jsDateFormat()
    {
        ...
        $format = str_replace("m", "MM", $format); // month
        $format = str_replace("d", "DD", $format);
        $format = str_replace("H", "HH", $format); // 24-hour
        $format = str_replace("h", "hh", $format); // 12-hour
        $format = str_replace("i", "mm", $format); // minutes
        $format = str_replace("s", "ss", $format); // seconds
        $format = str_replace("A", "A", $format);  // AM/PM
        return $format;
    }
