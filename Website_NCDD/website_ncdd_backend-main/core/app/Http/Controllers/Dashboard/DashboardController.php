...
class DashboardController extends Controller
{
    ...
    public function index()
    {
        ...
        // Analytics
        $TodayVisitors = AnalyticsVisitor::where('date', date('Y-m-d'))->count('id');
         
        $TodayPages = AnalyticsPage::where('date', date('Y-m-d'))->count('id');
        ...
        foreach ($AnalyticsVisitors as $AnalyticsV) {
            ...
            $TotalV = AnalyticsVisitor::where('date', '>=', $daterangepicker_start)
                ->where('date', '<=', $daterangepicker_end)
                ->where("$stat", $AnalyticsV->$stat)->count('id');
            ...
            $TotalP = 0;
            foreach (array_chunk($AllVArray, 500) as $chunk) {
                $TotalP += AnalyticsPage::whereIn('visitor_id', $chunk)
                    ->count('id');
            }
        ...
        foreach ($AnalyticsVisitors as $AnalyticsV) {
            ...
            $TotalV = AnalyticsVisitor::where('date', $date_today)->where("$stat", $AnalyticsV->$stat)->count('id');
            
            $AllVArray = AnalyticsVisitor::where('date', $date_today)
                ->select('id')->where("$stat", $AnalyticsV->$stat)
                ->get()
                ->toArray();
