                    ...
                    if ($KEY == "col_id") {
                        ...
                        if ($WebmasterSection->title_status) {
                            if (@Auth::user()->permissionsGroup->edit_status) {
                                ...
                                $Cell_Value = "<a href='" . route("topicsEdit", ["webmasterId" => @$Topic->webmasterSection->id, "id" => $Topic->id]) . "'>" . $photo . "<div class='h6 m-b-0 line-height-30'>" . $icon . $title . "</div>" . $section . $comments . "</a>";
                ...
                foreach ($Topic->attachFiles as $TFile) {
                    ...
                    foreach (Helper::languagesList() as $ActiveLanguage) {
                        if ($ActiveLanguage->box_status) {
                            ...
                            $new_file_name = "c" . $TFile->$formFileName;
                            $copied = File::copy($path . $TFile->$formFileName, $path . $new_file_name);
                            $AttachFile->$formFileName = $new_file_name;
                        }
                    }
                    if ($copied) {
        ...
        if (!empty($WebmasterSection)) {
            ...
            foreach (Helper::languagesList() as $ActiveLanguage) {
                if ($ActiveLanguage->box_status) {
                    ...
                    if ($request->$formFileName != "") {
                         $this->validate($request, [
                            $formFileName => 'required|mimes:' . $this->allowed_file_types
                        ]);
                    ...
                    if ($fileFinalName != "") {
                        ...
                        $AttachFile->$formFileName = $fileFinalName;
                    }else {
                        return redirect()->action('Dashboard\TopicsController@edit', [$webmasterId, $id])->with('activeTab', 'files');
                    }
                }
            ...
            if (!empty($AttachFile)) {
                foreach (Helper::languagesList() as $ActiveLanguage) {
                    if ($ActiveLanguage->box_status) {
                        ...
                        if ($request->$formFileName != "") {
                            $this->validate($request, [
                                $formFileName => 'mimes:' . $this->allowed_file_types
                            ]);
                            // Delete a Topic photo
