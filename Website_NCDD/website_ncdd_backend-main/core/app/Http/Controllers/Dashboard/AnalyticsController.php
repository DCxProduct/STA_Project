...
class AnalyticsController extends Controller
{
    ...
    public function index($stat = "date")
    {
        ...
        foreach ($AnalyticsVisitors as $AnalyticsV) {
            ...
            $TotalV = AnalyticsVisitor::where('date', '>=', $daterangepicker_start)
                ->where('date', '<=', $daterangepicker_end)
                ->where("$stat", $AnalyticsV->$stat)->count();

            $AllVArray = AnalyticsVisitor::where('date', '>=', $daterangepicker_start)
                ->where('date', '<=', $daterangepicker_end)
            ...
            foreach (array_chunk($AllVArray, 500) as $chunk) {
                $TotalP += AnalyticsPage::where('date', '>=', $daterangepicker_start)
                    ->where('date', '<=', $daterangepicker_end)
                    ->whereIn('visitor_id', $chunk)
                    ->count('id');
            }
    ...
    public function visitors()
    {
        ...
        $AnalyticsVisitors = AnalyticsVisitor::orderby('date', 'desc')->orderby('time','desc')->paginate(config('smartend.backend_pagination'));
