...
{
    private string $uploadPath = "uploads/feedbacks/";
...
For more details check <a href='http://smartfordesign.net/smartend/documentation/api.html' target='_blank'><strong>API documentation</strong></a>
    ...
    public function topics($section_id, $page_number = 1, $topics_count = 0, $lang = '')
    {
        if ($section_id > 0) {
            ...
            $Topics = Topic::where([['webmaster_id', '=', $section_id], ['status',
                1], ['expire_date', '>=', date("Y-m-d")], ['date', '<=', date("Y-m-d H:i:s")], ['expire_date', '<>', null]])->orWhere([['webmaster_id', '=', $section_id], ['status', 1], ['expire_date', null]])->orderby('row_no', 'asc')->orderBy('id', 'desc');
    ...
    public function category($cat_id, $page_number = 1, $topics_count = 0, $lang = '')
    {
        if ($cat_id > 0) {
            ...
            $Topics = Topic::where(function ($q) {
                $q->where([['status', 1], ['expire_date', '>=', date("Y-m-d")], ['date', '<=', date("Y-m-d H:i:s")], ['expire_date', '<>', null]])->orWhere([['status', 1], ['expire_date', null]]);
    ...
    public function topic($topic_id, $lang = '')
    {
        if ($topic_id > 0) {
            ...
            $Topics = Topic::where([['id', '=', $topic_id], ['status',
                1], ['expire_date', '>=', date("Y-m-d")], ['date', '<=', date("Y-m-d H:i:s")], ['expire_date', '<>', null]])->orWhere([['id', '=', $topic_id], ['status', 1], ['expire_date', null]])->orderby('row_no', 'asc')->orderBy('id', 'desc')->get();
    ...
    public function topic_maps($topic_id, $lang = '')
    {
        if ($topic_id > 0) {
            ...
            // Get topic details
            $Topics = Topic::where([['id', '=', $topic_id], ['status',
                1], ['expire_date', '>=', date("Y-m-d")], ['expire_date', '<>', null]])->orWhere([['id', '=', $topic_id], ['status', 1], ['expire_date', null]])->orderby('row_no', 'asc')->orderBy('id', 'desc')->get();
    ...
    public function user_topics($user_id, $page_number = 1, $topics_count = 0, $lang = '')
    {
        if ($user_id > 0) {
            ...
            $Topics = Topic::where([['created_by', '=', $user_id], ['status',
                1], ['expire_date', '>=', date("Y-m-d")], ['date', '<=', date("Y-m-d H:i:s")], ['expire_date', '<>', null]])->orWhere([['created_by', '=', $user_id], ['status', 1], ['expire_date', null]])->orderby('row_no', 'asc');
    ...
    public function commentSubmit(Request $request)
    {
        ...
        if($request->comment_document != ""){
            $this->validate($request, [
                ...
                'comment_document' => 'required|file|mimes:pdf,jpg,jpeg,png,gif,webp|max:20480'
            ]);
        }else{
            $this->validate($request, [
                //'api_key' => 'required',
        ...
        //if ($request->api_key == Helper::GeneralWebmasterSettings("api_key")) {
            ...
            $next_nor_no = Comment::where('topic_id', '=', config('dev.app.comment_id'))->max('row_no');
            if ($next_nor_no < 1) {
                $next_nor_no = 1;
            } else {
                $next_nor_no++;
            ...
            $Comment->document = $attachFileFinalName;
            $Comment->topic_id = config('dev.app.comment_id');
            ...
            // Topic details
            $Topic = Topic::where('status', 1)->find(config('dev.app.comment_id'));
            if (!empty($Topic)) {
                ...
                //     $recipient = explode(",", str_replace(" ", "", $site_email));
                //     //$message_details = __('frontend.name') . ": " . $request->comment_name . "<hr>" . __('frontend.document') . ": " . $request->comment_email . "<hr>" . __('frontend.comment') . ":<br>" . nl2br($request->comment_message);
                //     $message_details = __('frontend.name') . ": " . strip_tags($request->comment_name) . "<hr>" .
                //                         __('frontend.document') . ": <a href='" . asset('uploads/feedbacks/' . $attachFileFinalName) . "' target='_blank'>" . $attachFileFinalName . "</a><hr>" .
                //                         __('frontend.comment') . ":<br>" . nl2br(strip_tags($request->comment_message));
