<div class="tab-pane {{  ( Session::get('active_tab') == 'styleTab' || Session::get('active_tab') =="") ? 'active' : '' }}" id="tab-5">
    ...
    <div class="p-a-md col-md-12">
        ...
        <div class="form-group">
            ...
            <div class="form-group row">
                <div class="col-sm-6">
                    <label>{{ __('backend.imageBackground') }} : </label>
                    @if($Setting->style_bg_image!="")
                        <div>
                            <div>
                                <div id="image_bg" class="col-sm-8 box p-a-xs">
                                    <a target="_blank"
                                       href="{{ asset('uploads/settings/'.$Setting->style_bg_image) }}"><img
                                            src="{{ asset('uploads/settings/'.$Setting->style_bg_image) }}"
                                            class="img-responsive">
                                        {{ $Setting->style_bg_image }}
                                    </a>
                                    <br>
                                    <a onclick="document.getElementById('image_bg').style.display='none';document.getElementById('photo_delete').value='1';document.getElementById('undo').style.display='block';"
                                       class="btn btn-sm btn-default">{!!  __('backend.delete') !!}</a>
                                </div>
                                <div id="undo" class="col-sm-4 p-a-xs" style="display: none">
                                    <a onclick="document.getElementById('image_bg').style.display='block';document.getElementById('photo_delete').value='0';document.getElementById('undo').style.display='none';">
