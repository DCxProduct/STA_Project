                        ...
                        @foreach(Helper::languagesList() as $ActiveLanguage)
                            @if($ActiveLanguage->box_status)
                                <div class="form-group row">
                                    ...
                                    <div class="col-sm-10">
                                        {!! Form::text('title_'.@$ActiveLanguage->code,'', array('placeholder' => '','class' => 'form-control','required'=>'','maxlength'=>250, 'dir'=>@$ActiveLanguage->direction)) !!}
                                    </div>
                                </div>
                            @endif
                        @endforeach
                            ...
                            @if($ActiveLanguage->box_status)
                                <div class="form-group row">
                                    <label for="files_file"
                                        class="col-sm-2 form-control-label">{!!  __('backend.topicAttach') !!} {!! @Helper::languageName($ActiveLanguage) !!}
                                    </label>
                                    <div class="col-sm-10">
                                        {!! Form::file('file_'.@$ActiveLanguage->code, array('class' => 'form-control','id'=>'attach_file','required'=>'','accept'=>".".@str_replace(",",",.",@$allowed_file_types))) !!}
                                    </div>
                                </div>
                            @endif
                        @endforeach
                ...
                @if (Session::get('fileST') == "edit")
                    <div>
                        ...
                        @foreach(Helper::languagesList() as $ActiveLanguage)
                            @if($ActiveLanguage->box_status)
                                <div class="form-group row">
                                    ...
                                    <div class="col-sm-10">
                                        {!! Form::text('title_'.@$ActiveLanguage->code,Session::get('AttachFile')->{'title_'.@$ActiveLanguage->code}, array('placeholder' => '','class' => 'form-control','required'=>'','maxlength'=>250, 'dir'=>@$ActiveLanguage->direction)) !!}
                                    </div>
                                </div>
                            @endif
                        @endforeach
                        ...
                        @foreach(Helper::languagesList() as $ActiveLanguage)
                            @if($ActiveLanguage->box_status)
                                 <?php
                                    $file_var = "file_" . @$ActiveLanguage->code;
                                ?>
                                <div class="form-group row">
                                    <label for="files_file"
                                        class="col-sm-2 form-control-label">{!!  __('backend.topicAttach') !!} {!! @Helper::languageName($ActiveLanguage) !!}</label>
                                    <div class="col-sm-10">
                                        <div class="col-sm-4 box p-a-xs">
                                            <a target="_blank"
                                            href="{{ asset('uploads/topics/'.Session::get('AttachFile')->$file_var) }}"> {!! Helper::GetIcon(asset('uploads/topics/'),Session::get('AttachFile')->$file_var) !!} {{ Session::get('AttachFile')->$file_var}} </a>
                                        </div>
                                        {!! Form::file('file_'.@$ActiveLanguage->code, array('class' => 'form-control','id'=>'files_file','accept'=>".".@str_replace(",",",.",@$allowed_file_types))) !!}
                                    </div>
                                </div>
                            @endif
                        @endforeach
                ...
                @if(count($Topic->attachFiles)>0)
                    ...
                    <table class="table table-bordered">
                        ...
                        @foreach($Topic->attachFiles as $file)
                            ...
                            if ($file->$file_var != "") {
                                $file_url = $file->$file_var;
                            } else {
                                $file_url = $file->$file_var2;
                            }
                            ?>
                            <tr>
                                ...
                                <td>
                                    ...
                                    <a href="{{ asset('uploads/topics/'.$file_url) }}"
                                       target="_blank">
                                        {!! Helper::GetIcon(asset('uploads/topics/'),$file_url) !!}
                                        {{$file_url}}</a>
                                </td>
                                <td>
                                    <small>
