...
const PrakasCard = ({ title, date, href, visits, lang, expire_date, type }) => {
  ...
  return (
    <Link
      href={href}
      className="bg-white rounded-lg py-2 px-1 transform transition-transform duration-300 hover:scale-105 mt-1"
    >
      <div className="">
        <div>
          <h2 className={`md:line-clamp-2 line-clamp-3 lg:text-size-18 khmer-text-odor:${lang} min-h-14 max-h-16`}>
            {title}
          </h2>
        </div>
      </div>
      <div className={`md:flex items-center mt-2 text-gray-400 border-b-1px pb-2 khmer-text-batr:${lang} lg:text-size-15 text-size-13`}>
        <IoCalendarOutline size={20} className="" />
        <p className="md:ml-2 lg:text-size-15 text-size-13 ">
          {NumToKhmer(day, lang)}&nbsp;
        ...
        >
          ផុតកំណត់ {NumToKhmer(exp_day, lang)}&nbsp;
